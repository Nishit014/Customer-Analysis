...
if __name__ == "__main__":
    ...
    BALANCE = st.number_input("Balance amount left in their account to make purchases",step=1000)
    BALANCE_FREQUENCY = st.number_input("How frequently the Balance is updated(Frequency between 0 and 1)")
    PURCHASES = st.number_input("PURCHASES",step=1)
    ONEOFF_PURCHASES = st.number_input("Maximum purchase amount done in one-go",step=1)
    INSTALLMENTS_PURCHASES = st.number_input("Amount of purchase done in installment",step=1)
    CASH_ADVANCE = st.number_input("Cash in advance given by the user",step=1)
    ...
    CASH_ADVANCE_TRX = st.number_input("Number of Transactions made with cash in advance",step=1)
    PURCHASES_TRX = st.number_input("Number of purchase transactions made",step=1)
    CREDIT_LIMIT = st.number_input("CREDITLIMIT",step=1)
    PAYMENTS = st.number_input("PAYMENTS",step=1)
    MINIMUM_PAYMENTS = st.number_input("MINIMUM_PAYMENTS",step=1)
    PRC_FULL_PAYMENT = st.number_input("Percent of full payment paid by user",step=1)
    TENURE = st.number_input("Tenure of credit card service for user",step=1)
    if st.button("Predict"):

        result=Predict(BALANCE/50, BALANCE_FREQUENCY, PURCHASES, ONEOFF_PURCHASES/50,
        INSTALLMENTS_PURCHASES, CASH_ADVANCE/50, PURCHASES_FREQUENCY,
        ONEOFF_PURCHASES_FREQUENCY, PURCHASES_INSTALLMENTS_FREQUENCY,
        CASH_ADVANCE_FREQUENCY, CASH_ADVANCE_TRX, PURCHASES_TRX,
        CREDIT_LIMIT/50, PAYMENTS, MINIMUM_PAYMENTS, PRC_FULL_PAYMENT/100,
        TENURE)
        if result==0:
            st.write("Findings")
